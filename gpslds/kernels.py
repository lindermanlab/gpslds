...
class Kernel:
    ...
    #     return gaussian_int(fn, m, S, self.quadrature.weights, self.quadrature.unit_sigmas) # (K, )
    ...
    def E_Kxz(self, zs, m, S, kernel_params):
        """Computes E[k(x,z)] wrt q(x) = N(x|m,S) for z in zs."""
        sigmas = m + (jnp.linalg.cholesky(S) @ self.quadrature.unit_sigmas[...,None]).squeeze(-1) # (n_quad**K, K)
        
        fn = lambda x: vmap(partial(self.K, x, kernel_params=kernel_params))(zs) # returns (M,)
        def _step(carry, arg):
            weight, sigma = arg
            return carry + weight * fn(sigma), None

        init_carry = jnp.zeros(len(zs))
        arg = (self.quadrature.weights, sigmas)
        result, _ = lax.scan(_step, init_carry, arg)
        return result # (M,)

    def E_KzxKxz(self, zs, m, S, kernel_params):
        """Computes E[k(z1,x)k(x,z2)] wrt q(x) = N(x|m,S) for z1, z2 in zs."""
        sigmas = m + (jnp.linalg.cholesky(S) @ self.quadrature.unit_sigmas[...,None]).squeeze(-1) # (n_quad**K, K)

        _fn = lambda x, z1, z2: self.K(z1, x, kernel_params) * self.K(x, z2, kernel_params)
        fn = lambda x: vmap(vmap(partial(_fn, x), (None, 0)), (0, None))(zs, zs) # returns (M, M)
        def _step(carry, arg):
            weight, sigma = arg
            return carry + weight * fn(sigma), None

        init_carry = jnp.zeros((len(zs), len(zs)))
        arg = (self.quadrature.weights, sigmas)
        result, _ = lax.scan(_step, init_carry, arg)
        return result # (M, M)
        
    def E_dKzxdx(self, zs, m, S, kernel_params):
        """Computes E[dk(z,x)/dx] wrt q(x) = N(x|m,S) for z in zs."""
        sigmas = m + (jnp.linalg.cholesky(S) @ self.quadrature.unit_sigmas[...,None]).squeeze(-1) # (n_quad**K, K)
        
        fn = lambda x: vmap(grad(partial(self.K, x, kernel_params=kernel_params)))(zs) # returns (M, K)
        def _step(carry, arg):
            weight, sigma = arg
            return carry + weight * fn(sigma), None

        init_carry = jnp.zeros((len(zs), len(m)))
        arg = (self.quadrature.weights, sigmas)
        result, _ = lax.scan(_step, init_carry, arg)
        return result # (M, K)
